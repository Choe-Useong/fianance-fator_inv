...
# 차트 저장 (README에서 참조 가능)
try:
    plot_idx = nav.index.intersection(bench_nav.index)
    fig, ax = plt.subplots(figsize=(10, 4))
    nav.loc[plot_idx].plot(label="Portfolio", ax=ax)
    bench_nav.loc[plot_idx].plot(label="Benchmark", ax=ax)
    ax.legend()
    ax.set_title("Portfolio vs Benchmark")
    fig.tight_layout()
    fig.savefig(docs_dir / "portfolio_vs_benchmark.png", dpi=150)
    plt.close(fig)
except Exception as e:
    print("[WARN] 차트 저장 실패:", e)
...
try:
    def _ann_metrics(r):
        ...
        sharpe = (float(cagr) / vol) if vol and vol > 0 else None
        return (float(cagr), float(vol), float(sharpe) if sharpe is not None else None)

    cagr_p, vol_p, sh_p = _ann_metrics(port_ret)
    cagr_b, vol_b, sh_b = _ann_metrics(bench_ret)

    start = str(nav.index.min().date()) if not nav.empty else ""
    end = str(nav.index.max().date()) if not nav.empty else ""
    rebal_cnt = int(weights.notna().any(axis=1).sum())

    final_nav_p = float(nav.dropna().iloc[-1]) if not nav.dropna().empty else None
    final_nav_b = float(bench_nav.dropna().iloc[-1]) if not bench_nav.dropna().empty else None

    lines = [
        "# Backtest Summary",
        f"Period: {start} ~ {end}",
        f"Rebalances: {rebal_cnt}",
        (f"Final NAV (Portfolio): {final_nav_p:.6f}" if final_nav_p is not None else "Final NAV (Portfolio): NA"),
        (f"Final NAV (Benchmark): {final_nav_b:.6f}" if final_nav_b is not None else "Final NAV (Benchmark): NA"),
        "",
        "Annualized metrics (rf=0):",
        (f"- Portfolio: CAGR={cagr_p:.4%} | Vol={vol_p:.4%} | Sharpe={sh_p:.2f}" if cagr_p is not None else "- Portfolio: NA"),
        (f"- Benchmark: CAGR={cagr_b:.4%} | Vol={vol_b:.4%} | Sharpe={sh_b:.2f}" if cagr_b is not None else "- Benchmark: NA"),
        "",
        "Generated by 코드/9_백테스트.py",
    ]
    (docs_dir / "backtest_summary.md").write_text("\n".join(lines), encoding="utf-8")
except Exception as e:
    print("[WARN] 요약 저장 실패:", e)
